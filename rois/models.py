...
class LabelSet(models.Model):
    ...
    image_id_list = ArrayField(models.CharField(max_length=128),null=True)
...
class Image(models.Model):
    ...
    camera = models.ForeignKey('PlanktonCamera',on_delete=models.PROTECT,null=True)
    ...
    def import_image(self,path,from_raw=False):
        ...
        if self.image_id.split('.')[1] == 'tif':
            ...
            if from_raw:
                ...
                img = cvtools.import_image(path,self.image_id,raw=True)
            else:
...
class LabelInstance(models.Model):
    ...
    label_set = models.ForeignKey(LabelSet, on_delete=models.CASCADE,related_name='label_sets')
    label = models.ForeignKey(Label, on_delete=models.PROTECT,related_name='label_instances')
    annotator = models.ForeignKey(Annotator, on_delete=models.PROTECT,related_name='label_instances')
