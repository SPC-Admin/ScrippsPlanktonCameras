...
urlpatterns = [
        ...
        url(r'annotators/$',views.annotators,name='annotators'),
...
#        url(r'get_histograms/$',views.get_historgrams,name='get_histograms'),
        ...
        url(r'map_server_api$',views.map_server_api,name='map_server_api'),

        url(r'images/(?P<camera>.+)/(?P<utcstart>.+)/(?P<utcend>.+)/(?P<hourstart>.+)/(?P<hourend>.+)/(?P<nimages>.+)/(?P<minlen>.+)/(?P<maxlen>.+)/(?P<minaspect>.+)/(?P<maxaspect>.+)/(?P<exclude>.+)/(?P<ordering>.+)/(?P<archive>.+)/(?P<label>.+)/(?P<labeltype>.+)/(?P<tag>.+)/(?P<annotator>.+)/$',views.ImageList.as_view()),
