...
from map_reduce import map_request, reduce_result
...
def map_server_api(request):
    ...
    if (request.is_ajax() and request.method == 'POST'):
        ...
        if 'server_list' in data and 'api_path' in data:

            resp = map_request(data['server_list'],data['api_path'])
            
            if 'data_reducer' in data:
                resp = reduce_result(resp,data['data_reducer'])
        ...
        return HttpResponse(json.dumps({'error':'No json input.'}),content_type="application.json")
...
class ImageList(generics.ListAPIView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        response.data['query_time'] = time.time() - start_time
