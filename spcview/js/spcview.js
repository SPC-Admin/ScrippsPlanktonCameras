var spcview = (function() {
    ...
    var cameraID = 0;
    var loadedCounter = 0;
    var cameraNames = ["SPC2","SPCP2"];
    var imagePostfix = ['.jpg','_binary.png','_boundary.png','.png'];
    var imagePostfixIndex = 0;
    var cameraRes = [7.38/1000,0.738/1000,7.38/1000,0.738/1000];
    var dataLoaded = false;
    var siteURL = "http://fiona-jaffe-01.ucsd.edu";
    var appBase = siteURL + '/data/';
    var viewerBase = siteURL + '/viewer/spcview.html';
    var roiImagesBase = "rois/images/";
    var roiArchiveBase = appBase + "rois/imagearchive/";
    var dailyStatsBase = appBase + "roistats/dailystats/";
    var dailyStats = [];
    var allLabels = [];
    var allTags = [];
    ...
    var statusBarContiner = $('#status-bar-container');
    ...
    var queryPresets = [
        {
            "name": "spc2-small",
            "label": "SPC2 Small [0.5,1.0]",
            "title": "Images in the range 0.5 mm to 1.0 mm",
            "camera": "SPC2",
            "minmaj": .5,
            "maxmaj": 1,
            "minasp": 0.05,
        ...
        {
            "name": "spc2-large",
            "label": "SPC2 Large [1.0,5.0]",
            "title": "Images in the range 1.0 mm to 5.0 mm",
            "camera": "SPC2",
            "minmaj": 1,
            "maxmaj": 5.0,
            "minasp": 0.3,
        ...
        {
            "name": "spc2-circular-small",
            "label": "SPC2 Circular [0.5,1.0]",
            "title": "Circular images in the range 0.5 to 1.0 mm",
            "camera": "SPC2",
            "minmaj": .5,
            "maxmaj": 1.0,
            "minasp": 0.9,
        ...
        {
            "name": "spc2-circular-big",
            "label": "SPC2 Circular [1.0,5.0]",
            "title": "Circular images in the range 1.0 to 5.0 mm",
            "camera": "SPC2",
            "minmaj": 1.0,
            "maxmaj": 5.0,
            "minasp": 0.9,
            "maxasp": 1.0,
                ...
                {
            "name": "spcp2-small",
            "label": "SPCP2 Small [0.05,0.1]",
            "title": "Images in the range 0.05 mm to 0.1 mm",
            "camera": "SPCP2",
            "minmaj": .05,
            "maxmaj": 0.1,
            "minasp": 0.05,
        ...
        {
            "name": "spcp2-large",
            "label": "SPCP2 Large [.05,1.0]",
            "title": "Images in the range 0.05 to 1.0 mm",
            "camera": "SPCP2",
            "minmaj": .05,
            "maxmaj": 1.0,
            "minasp": 0.05,
        ...
        {
            "name": "spcp2-circular-small",
            "label": "SPCP2 Circular [0.05,0.1]",
            "title": "Circular images in the range 0.05 to 0.1 mm",
            "camera": "SPCP2",
            "minmaj": .05,
            "maxmaj": 0.1,
            "minasp": 0.9,
        ...
        {
            "name": "spcp2-circular-big",
            "label": "SPCP2 Circular [1.0,5.0]",
            "title": "Circular images in the range 0.05 to 1.0 mm",
            "camera": "SPCP2",
            "minmaj": 0.5,
            "maxmaj": 1.0,
            "minasp": 0.9,
    ...
    function renderQueryString(JSONData) {
        ...
        out = "<b>Query ("+JSONData.query_time.toFixed(2)+" s) : Major Axis (mm) ["+minMaj.val()+","+maxMaj.val()+"],";
    ...
    function showImageDetail(data) {
        ...
        $('#DownloadImage').on('click', data, function (event) {
            ...
            var image_url = siteURL + event.data.image_url + '.zip';
    ...
    function reduceImageData(JSONData) {
        
        // reduce images from multiple servers
        image_data = [];
        allServers = JSONData.server_url;
        max_count_index = 0;
        max_count = 0;
        for (var i = 0; i < allServers.length; i++) {
            for (var j = 0; j < JSONData.data[i].image_data.results.length; j++) {
                tmp = JSONData.data[i].image_data.results[j];
                tmp.image_url = allServers[i] + '/' + tmp.image_url;
                image_data.push(tmp);
            }
            if (JSONData.data[i].image_data.count > max_count) {
                max_count_index = i;
                max_count = JSONData.data[i].image_data.count;
            }
        }

        // Populate the additional fields for page navigation and other
        
        reduced = {};
        reduced.image_data = {};
        reduced.image_data.results = image_data;
        reduced.query_time = JSONData.data[max_count_index].query_time;
        reduced.page_size = JSONData.data[max_count_index].page_size;
        reduced.image_data.count = JSONData.data[max_count_index].image_data.count;
        reduced.image_data.next = JSONData.data[max_count_index].image_data.next;
        reduced.image_data.previous = JSONData.data[max_count_index].image_data.previous;
    ...
    function renderMosaicfromJSON(JSONData) {        
        ...
        image_data = JSONData.image_data.results;
        ...
        image_data = image_data.sort( function (a,b) {
            return b.image_height - a.image_height;
        });

        for ( var i = 0; i < image_data.length; i++ ) {
            var elem = getItemElement(image_data[i]);
        ...
        function getItemElement(data) {
            ...
            var img = new Image();
            img.src = data.image_url+".jpg";
            img.onload = function() {
                loadedCounter++;
                elem.style.backgroundImage = "url('"+img.src+"')";
                var pct = Math.floor(100*loadedCounter/image_data.length);
                statusBar.width(pct.toString()+"%");
                if (loadedCounter == image_data.length) {
                    statusBar.html(renderQueryString(JSONData));
                    //statusBarContiner.fadeOut(5000);
                }
                else {
                    statusBar.html("Loaded "+loadedCounter.toString()+" of "+image_data.length.toString()+" images");
                }
            };
    ...
    // build preset menu
        $.each(queryPresets, function(index,preset) {
            ...
            var a = $('<a/>')
                .addClass('ui-all')
                .attr('href', viewerBase+'?preset='+preset.name)
                .text(preset.label)
                .appendTo(li);
        });
    };
    ...
    my.loadRoiData = function() {
        ...
        $.ajax({
            ...
            url: appBase + 'rois/map_server_api',
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
            processData: false,
            data: JSON.stringify({
                'server_list': ['http://spc/ucsd.edu','http://planktivore.ucsd.edu'],
                'api_path': 'data/'+lastQueryUrl
            }),
            success: function(json) {
                dataLoaded = true;

                //console.log(json);
                json = reduceImageData(json);
    ...
    my.getPage = function(pageNumber) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                'server_list': ['http://spc/ucsd.edu','http://planktivore.ucsd.edu'],
                'api_path': 'data/'+lastQueryUrl + '?page=' + pageNumber
            }),
            // handle a successful response
            success : function (json) {
                dataLoaded = true;
    ...
    runNamedSearch.on( "click", function() {
        ...
        $.ajax({
            url : siteURL + "/data/rois/find_image/" + name, // the endpoint
            type : "GET", // http method
            // handle a successful response
            success : function (json) {
                //console.log(json);
